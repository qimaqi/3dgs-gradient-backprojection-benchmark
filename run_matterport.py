...
if __name__ == "__main__":
    ...
    for scene in validation_set:
        # scene = scene.split('/')[-1]
        if args.rescale == 0:
            potential_results_dir = os.path.join('/usr/bmicnas02/data-biwi-01/qimaqi_data/workspace/neurips_2025/3dgs-gradient-backprojection-benchmark/results/matterport', scene, 'features_lseg_512_640.pt')
            potential_xyz_dir = os.path.join('/usr/bmicnas02/data-biwi-01/qimaqi_data/workspace/neurips_2025/3dgs-gradient-backprojection-benchmark/results/matterport', scene, 'xyz_lseg_512_640.npy')
        # elif args.rescale == 1:
        ...
        if os.path.exists(potential_results_dir) and os.path.exists(potential_xyz_dir):
            ...
            continue   
        print(scene)
        os.system("python backproject_ply_matterport.py --scene_name {} --rescale {}".format(scene, rescale))

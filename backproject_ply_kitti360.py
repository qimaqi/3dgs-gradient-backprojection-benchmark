...
from utils import (
    ...
    prune_by_gradients_kitti360,
    test_proper_pruning_kitti360,
)



def create_feature_field_lseg(splats, batch_size=1, use_cpu=False, inverse_extrinsics=True, resize=[188, 704]): # resize 704, 
...
def main(
    data_root_path: str = "/srv/beegfs-benderdata/scratch/qimaqi_data/data/neurips_2025/kitti_360_subset_chunk/", # subset
    ...
    ply_root_path: str = "/srv/beegfs-benderdata/scratch/qimaqi_data/data/neurips_2025/kitti_360_subset_chunk/",  # checkpoint path, can generate from original 3DGS repo
    results_root_dir: str = "./results/kitti360/",  # output path
    rasterizer: Literal[
        "inria", "gsplat"
    ...
    scene_i: str = "2013_05_28_drive_0000_sync/0000000599_0000000846/",  # Scene name
    ...
    ply_path_i = os.path.join(ply_root_path, scene_i, 'output_dynamic_mask', 'ckpts', 'point_cloud_10000.ply')
    data_dir = os.path.join(data_root_path, scene_i)
    splats = load_ply(
        ply_path_i, data_dir, rasterizer=rasterizer, dataset="kitti360", max_sh_degree=0
    )
    splats_optimized = prune_by_gradients_kitti360(splats)

    test_proper_pruning_kitti360(splats, splats_optimized)

    # raise Exception("Debugging")
    splats = splats_optimized
    if feature == "lseg":
        if rescale == 0:
            feat_save_path = f"{result_i_dir}/features_lseg_188_704.pt"
            if not os.path.exists(feat_save_path):
                features = create_feature_field_lseg(
                    splats, feature_field_batch_count, run_feature_field_on_cpu,resize=[188, 704]
                )
                torch.save(features, feat_save_path)

            xyz_save_path = f"{result_i_dir}/xyz_lseg_188_704.npy"
...
def get_arguments():
    ...
    argparser.add_argument(
        "--scene_name",
        type=str,
        default="2013_05_28_drive_0000_sync/0000000599_0000000846",

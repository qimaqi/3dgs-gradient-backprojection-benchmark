...
def parse_args():
    ...
    parser.add_argument("--val_split_path", type=str, default="/usr/bmicnas02/data-biwi-01/qimaqi_data/workspace/neurips_2025/3dgs-gradient-backprojection-benchmark/splits/scannetpp_mini_val.txt")
    parser.add_argument("--preprocessed_root", type=str, default="/srv/beegfs02/scratch/qimaqi_data/data/scannetpp_pointcloud")
    parser.add_argument("--results_root", type=str, default= "/usr/bmicnas02/data-biwi-01/qimaqi_data/workspace/neurips_2025/3dgs-gradient-backprojection-benchmark/results/scannetpp_rebuttal")
    #  "/srv/beegfs-benderdata/scratch/qimaqi_data/data/gaussianworld/3dgs-gradient-backprojection-benchmark-results/scannetpp_save/"
    parser.add_argument("--label_path", type=str, default="/usr/bmicnas02/data-biwi-01/qimaqi_data/workspace/neurips_2025/language_feature_experiments/metadata/scannetpp_semseg_top100.txt")
...
def main():
    ...
    text_feat_lseg = text_feat_lseg.detach().cpu().numpy()  # Convert to numpy for saving
    print("text_feat_lseg shape: ", text_feat_lseg.shape)
    np.save(os.path.join("./scanetpp100_text_embeddings_lseg_clip.npy"), text_feat_lseg)
    ...
    for scene_id in tqdm(scene_ids, desc="Scene", dynamic_ncols=True):
        # xyz_path = args.xyz_path.replace("09c1414f1b", scene_id)
        # langfeat_path = args.langfeat_path.replace("09c1414f1b", scene_id)
        ...
        # gs_path = os.path.join(args.gs_root, scene_id, "ckpts", "point_cloud_30000.ply")
        # gauss_xyz, _ = read_ply_file_3dgs(gs_path)
        gauss_xyz = np.load(xyz_path)  # (G, 3)
        assert gauss_lang_feat.shape[0] == gauss_xyz.shape[0], f"Mismatch in number of points: {gauss_lang_feat.shape[0]} vs {gauss_xyz.shape[0]}"
        # print("pred_feat shape: ", pred_feat.shape)

        # raise NotImplementedError("Please use the correct path for the features.")
        ...
        # print("xyz_val stastics: ", xyz_val.shape)
        # print("xyz_val x min: ", xyz_val[:, 0].min(), "x max: ", xyz_val[:, 0].max())
        # print("xyz_val y min: ", xyz_val[:, 1].min(), "y max: ", xyz_val[:, 1].max())
        # print("xyz_val z min: ", xyz_val[:, 2].min(), "z max: ", xyz_val[:, 2].max())
        ...
        #     continue
        # 
        # print("gauss_xyz shape: ", gauss_xyz.shape)

        print("gauss_xyz stastics: ", gauss_xyz.shape)
        print("gauss_xyz x min: ", gauss_xyz[:, 0].min(), "x max: ", gauss_xyz[:, 0].max())
        print("gauss_xyz y min: ", gauss_xyz[:, 1].min(), "y max: ", gauss_xyz[:, 1].max())
        print("gauss_xyz z min: ", gauss_xyz[:, 2].min(), "z max: ", gauss_xyz[:, 2].max())

        # angfeat_path = .langfeat_path
        ...
        nn_lang_feat = gauss_lang_feat[torch.from_numpy(nn_idx)]
        ...
        save_fname = f"backproject_lseg_{scene_id}_semseg_pred.npy"
        save_path = os.path.join(args.results_root, scene_id, save_fname)
        ...
        # np.save(os.path.join(args.results_root, scene_id, "pred_label.npy"), pred_label)

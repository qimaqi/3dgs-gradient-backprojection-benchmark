...
def get_arguments():
    ...
    parser.add_argument('--split', type=str, default='/usr/bmicnas02/data-biwi-01/qimaqi_data/workspace/neurips_2025/3dgs-gradient-backprojection-benchmark/splits/kitti360_mini_val.txt', help='Split name')
...
if __name__ == "__main__":
    ...
    for scene in validation_set:
        # scene = scene.split('/')[-1]
        if args.rescale == 0:
            potential_results_dir = os.path.join('/usr/bmicnas02/data-biwi-01/qimaqi_data/workspace/neurips_2025/3dgs-gradient-backprojection-benchmark/results/kitti360/', scene, 'features_lseg_188_704.pt') # 704, 188
        # elif args.rescale == 1:
        ...
        if os.path.exists(potential_results_dir):
            ...
            continue   
        print(scene)
        os.system("python backproject_ply_kitti360.py --scene_name {} --rescale {}".format(scene, rescale))
    end = time.time()
